...
func (m *Consul) Init() {
	...
	if m.config.InConfig("consul") {
		...
		configKey := fmt.Sprintf("%s/%s,%s/data", m.config.GetString("consul.config.prefix"), m.config.GetString("spring.application.name"), m.config.GetString("spring.profiles.active"))
